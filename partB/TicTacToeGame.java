...
public class TicTacToeGame
{
	...
	public static void printGameInstructions() {
		System.out.println("Welcome to the tic-tac-toe game. This is a game for two players, represented by X and O pieces.");
        System.out.println("The players will take turns marking down their piece in a 3x3 grid. The player who succeeds in placing three of their pieces in a diagonal, horizontal, or vertical row is the winner.");
	...
	//view player records
	public static void viewRecords(TicTacToePlayer player) {
		System.out.println("\nPlayer name: " + player.getName());
		System.out.println("Player piece: " + player.getPlayerPiece());
	...
	//start new game
	public static void newGame(TicTacToePlayer player1, TicTacToePlayer player2) {
    ...
    public static int move(SquareBoard currentBoard, TicTacToePlayer player) {
        ...
        currentBoard.setPiece(player.getPlayerPiece(), row, column);
    ...
    public static boolean checkFreeSpace(SquareBoard currentBoard, int x, int y) {
    	XOPiece blankSpace = new XOPiece();

    	/*System.out.println("FOR TESTING:");
    	boolean check = currentBoard.getPiece(x,y).match(blank);
    	System.out.println("current player move match blank: " + check);
    	check = currentBoard.getPiece(0,0).match(blank);
    	System.out.println("[0,0] matches blank space: " + check);
    	check = playerPiece.match(blank);
    	System.out.println("playerpiece matches blank space: " + check);*/

    	boolean result = currentBoard.getPiece(x,y).match(blankSpace);
    	if (!result) {
	...
	*/
    public static int checkWin(SquareBoard currentBoard, int x, int y, TicTacToePlayer player) {
    	int boardSize = 3;
    	XOPiece playerPiece = player.getPlayerPiece();

    	//check column (if win is horizontal)
        for (int column = 0; column < boardSize; column++) {
        	//if any space in row x does not equal to given player token piece, break out of for loop  
            if (!(currentBoard.getPiece(x, column).match(playerPiece))) {
            	break;
            //if by the last column all spaces in row x equal given player token piece, return player win
        ...
        for (int row = 0; row < boardSize; row++) {
        	//if any space in column x does not equal to given player token piece, break out of for loop  
            if (!(currentBoard.getPiece(row, y).match(playerPiece))) {
            	break;
            //if by the bottom row all spaces in column x equal given player token piece, return player win
            } if (row == boardSize-1) {
            	return 1;
            }   
        }
        ...
        if (x == y) {
        	...
        	for (int i = 0; i < boardSize; i++) {
        		//if any diagonal does not equal to given player token piece, break out of for loop
        		if (!(currentBoard.getPiece(i, i).match(playerPiece))) {
        			break;
        		//if by the last diagonal (bottom rightmost) all spaces equal given player token piece, return player win
        		} if (i == boardSize-1) {
        			return 1;
        		}
        	}
        ...
        if (x + y == boardSize-1) {
        	...
        	for (int i = 0; i < boardSize; i++) {
        		//starting from the top right, if any anti diagonal does not equal given player token piece, break out of for loop
        		if (!(currentBoard.getPiece(i,(boardSize-1)-i).match(playerPiece))) {
					break;
				//if by the last diagonal (bottom leftmost) all spaces equal given player token piece, return player win
				} if (i == boardSize-1) {
					return 1;
				}
        	}
    ...
    public static void main(String[] args)
    {
		...
		player1 = new TicTacToePlayer(givenName);

		System.out.println("Please enter Player 2's name:");
		givenName = input.nextLine();
		player2 = new TicTacToePlayer(givenName);


		//Assign an XOPiece to each TicTacToePlayer.
		char givenChoice = 'A';
		Exception invalidInput = new Exception("Invalid input.");
		while (givenChoice != 'X' && givenChoice != 'O' &&
			   givenChoice != 'x' && givenChoice != 'o') {
			try {
				System.out.println("For Player 1/" + player1.getName() + ", choose either the X or O piece:");
				givenChoice = input.nextLine().charAt(0);

				//if player input is not X or O, throws invalidInput exception
				if (givenChoice != 'X' && givenChoice != 'O' &&
					givenChoice != 'x' && givenChoice != 'o') {
					throw invalidInput;
				}
		...
		player1.setPlayerPiece(givenChoice);


		//if player 1 entered X, player 2 automatically assigned O
		if (givenChoice == 'X' || givenChoice == 'x') {
			System.out.println("Player 1/" + player1.getName() + " is assigned X. Player 2/" + player2.getName() + " is assigned O.");
			player2.setPlayerPiece('O');
		//if player 1 entered O, player 2 automatically assigned X
		} else {
			System.out.println("Player 1/" + player1.getName() + " is assigned O. Player 2/" + player2.getName() + " is assigned X.");
			player2.setPlayerPiece('X');
		}
            ...
            try {
    			...
    			if (userChoice == 1) {
                    ...
                    while (repeatGame == 'y') {
                        ...
                        //after new game, resets moveCount
                        moveCount = 0;
